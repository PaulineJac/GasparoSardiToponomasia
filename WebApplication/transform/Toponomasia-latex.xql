...
module namespace model="http://www.tei-c.org/pm/models/Toponomasia/latex";
...
\begin{titlepage}
	...
	{\centering
		\vspace{1cm}
		{\LARGE{Gasparis Sardi Ferrariensis}}\\
		\vspace{2cm}
			 \rput[r](-3pt,3pt){\pgfornament[scale=.35]{72}} 
			 {\scshape\Huge{Toponomasia}}
			 \rput[l](3pt,3pt){\pgfornament[scale=.35]{73}}\\ \rput(0,0){\pgfornament[scale=.5]{85}}\\
			 	\vspace{1cm}
		{\Large\scshape{LIBRI XVIII}}\\
	\vspace{2cm}
		{\large\itshape{In quibus explicantur eorum locorum nomina, \\qui illa immutarunt :  Vrbium etiam insignium origines \\
			Cum duplici Indice Antiquorum, et Nouorum Nominum \\
			Et tertio Rerum, quæ sparsim in opere continentur.
}}\\
\vspace{9cm}
		  \tikz[scale=.5] \callornament{am1.pgf}  ; \\
			 	\vspace{0.5cm}
		{\normalsize{Edition compiled from Cod. 174 of the Burgerbibliothek of Berne, by Pauline Jacsont.
}}
...
\newpage
\end{titlepage}
`{string-join($config?apply-children($config, $node, $params?to))}`
...
declare function model:transform($options as map(*), $input as node()*) {
    ...
    let $config :=
        map:merge(($options,
            map {
                "output": ["latex","print"],
                "odd": "/db/apps/tei-publisher/odd/Toponomasia.odd",
                "apply": model:apply#2,
